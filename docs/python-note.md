...
- [语法](#语法)
    ...
    - [魔术方法](#魔术方法)
        - [类创建初始化](#类创建初始化)
            ...
            - [初始化descriptor `__set_name__`](#初始化descriptor-__set_name__)
        - [元类相关](#元类相关)
            - [定义子类时回调 `__init_subclss__`](#定义子类时回调-__init_subclss__)
            - [`__isinstancecheck__` `__subclasscheck__`](#__isinstancecheck__-__subclasscheck__)
        - [删除相关](#删除相关)
            - [`__del__` `__delattr__`](#__del__-__delattr__)
        - [`__exit__` 自动关闭回收](#__exit__-自动关闭回收)
        - [可迭代 `__iter__`](#可迭代-__iter__)
        - [可调用对象 `__call__`](#可调用对象-__call__)
        - [属性监控回调 `__getattr__` `__getattribute__`](#属性监控回调-__getattr__-__getattribute__)
        - [可运算 操作符重载](#可运算-操作符重载)
            - [加减乘除](#加减乘除)
            - [逻辑运算](#逻辑运算)
            - [取整数](#取整数)
            - [相等比较 `==`, `is`, `__eq__`, ` id()` , `hash()`](#相等比较--is-__eq__--id--hash)
...
# 某个类 实现了 __bool__, 可以判断布尔值, 若找不到, 或者 __len__ 也可以用来判断布尔值, 返回 0 则是 false
...
class DirSize:
    # self 是 descriptor 对象本身, 也就是 Dir 中的 size
    # ins 是 Dir 对应的对象, 也就是 cur_dir
    #  cls 是 ins 的类型, 即 Dir
...
del cur_dir.size # 会调用 DirSize 中的 __del__
...
class Descriptor:
    # self 描述器自身
    # inscls 描述器对象所在的 class
    # attr 描述器赋值给的类变量名
    def __set_name__(self, inscls, attr):
        pass
...
#### 定义子类时回调 `__init_subclss__`

```python

class Base:
    # 在子类定义时被调用
    def __init_subclss__(sub_cls, name):
        # 为子类定义一个类变量
        sub_cls.x = {}
...
#### `__isinstancecheck__` `__subclasscheck__`

```python
# isinstance(xxx_obj, cls) 时调用
def __isinstancecheck__(self, ins) -> bool:
    pass

# issubclass(xxx_sub_cls, root_cls) 时调用
...
```python
# 在对象内存被释放时触发执行
# 触发时机不定, 因为对象内存可能在引用为 0, 也可能在 GC 时释放
#       所以del xx_obj  并不一定触发 xx_obj.__del__(), 只是减少了一个引用

#  __delattr__ 在 del obj.xxx  删除对象属性时被 call
默认实现:
...
o = Multiplier(3) # 通过 o 传入任何参数, 都会被乘以 3
o(4)  # 4 * 3    
```
...
# --------------------------------- 特殊的
# 以加法为例子: 若仅仅为自定义类 T 定义 __add__, 那么对于 t = T() 和另外类型的对象计算, 只能使用 t + xxx, 而不能 xxx + t, 若要互相能交换位置, 需要实现 __radd__
...
#### 相等比较 `==`, `is`, `__eq__`, ` id()` , `hash()`

```py
# 默认下, 比较两个自定义对象是否相等  obj1 == obj2, 是通过 obj1 is obj2, 比较的是内存地址
# 可自定义 __eq__改变 '==' 的逻辑

# __eq__, __ne__ 是一对, 只需要实现 __eq__
# __gt__, __lt__ 是一对, __ge__, __le__ 是一对


# 若自定义了 __eq__, 默认的 __hash__ 会被删掉, 所以若想作为字典 key, 必须同时自定义 __hash__
# 若何自定义 __hash__?
class A:
    def __hash__(self):
        # 将对象属性组成 tuple, 通过内置 hash()计算一个自定义 hash 值
        return hash(
            (self.attr1, self, att2, self.att3...)
        )
...
__reversed__(self) # 倒序, revered(lst) 时执行

__contains__(self, item)  #是否包含, if xx in lst 时执行


__iter__(self)  # 返回自定义对象的迭代器, 是的对象可以用在 for ele in lst



__missing__(self, key) -> xx:   # 定义在 dict 子类里, 若某个 key 不存在, 会调用
```
...
# 关闭/回收资源
__exit__(self, ex_type, ex_value, traceback)
