...
- [6. kafka](#6-kafka)
  ...
  - [6.3. 如何使用kafka](#63-如何使用kafka)
    - [6.3.1. kafka配置文件](#631-kafka配置文件)
    - [6.3.2. KafkaManager web 界面](#632-kafkamanager-web-界面)
    - [6.3.3. springboot 集成 kafka](#633-springboot-集成-kafka)
    - [6.3.4. 创建主题](#634-创建主题)
    - [6.3.5. 生产者发送](#635-生产者发送)
      - [6.3.5.1. kafka template 同步 异步](#6351-kafka-template-同步-异步)
      - [6.3.5.2. 生产者配置](#6352-生产者配置)
      - [6.3.5.3. 生产者事务消息](#6353-生产者事务消息)
    - [6.3.6. 消费者消费](#636-消费者消费)
      - [6.3.6.1. 配置 过滤器 全局异常处理](#6361-配置-过滤器-全局异常处理)
      - [6.3.6.2. 监听消费](#6362-监听消费)
        - [6.3.6.2.1. @kafkaListener基本使用](#63621-kafkalistener基本使用)
        - [6.3.6.2.2. 消费指定分区](#63622-消费指定分区)
        - [6.3.6.2.3. 动态开启关闭监听](#63623-动态开启关闭监听)
        - [6.3.6.2.4. 禁用自启动](#63624-禁用自启动)
        - [6.3.6.2.5. 定时启停监听器](#63625-定时启停监听器)
      - [6.3.6.3. 通过消费者消费](#6363-通过消费者消费)
      - [6.3.6.4. 消费者错误处理 单消息 批量消息](#6364-消费者错误处理-单消息-批量消息)
      - [6.3.6.5. offset 提交](#6365-offset-提交)
      - [6.3.6.6. 消息序列化](#6366-消息序列化)
      - [6.3.6.7. 自定义代替默认线程池](#6367-自定义代替默认线程池)
      - [6.3.6.8. 消费者组 多线程消费](#6368-消费者组-多线程消费)
      - [6.3.6.9. 消息转发](#6369-消息转发)
  - [6.4. 参数调优](#64-参数调优)
