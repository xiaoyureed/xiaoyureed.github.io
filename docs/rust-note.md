...
```rust
- 若没有捕获任何变量, 则实现 FnOnce (也就是 compiler 自动生成的结构体会实现 fnonce)
    compiler 会将 FnOnce 当成 fn(T) 函数指针看待
- 若有捕获变量, 同时对变量进行了修改, 则会实现 FnMut
- 若有捕获变量, 不会对变量进行修改, 则实现 Fn


Fn/FnMut/FnOnce 这个三个 trait 的关系是依次继承, 正好对应于语义三件套
