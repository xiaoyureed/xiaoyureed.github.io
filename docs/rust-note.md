...
/// RefCell<T> 可以获取内部不可变数据的可变引用(从而实现修改)
// - 将借用规则从编译期推迟到运行期，但是并不会绕过借用规则，当不符合时，程序直接panic
// 
...
// 非装箱 (Unboxed) 闭包 (现在的实现方式): 支持闭包按值和按引用绑定环境变量 ; 支持三种不同的闭包访问， 对应 self、&self和&mut self三种方法
// 将函数调用抽象为三种, 新增三个 trait 表示
// 
// 这样, 方法调用 a(b, c, d)变为一下三种执行方式:
// 
// Fn::call(&a, (b, c, d))          - 调用 参数为 &self， 这意味着它会对方法接收者进行不可变借用 ，也就是说，这种方法调用可以被调用多次。
// FnMut::call_mut(&mut a, (b, c, d)) - 调用参数为& mut self，这意味着它会对方法接收者进行可变借用
// FnOnce::call_once(a, (b, c, d)) - 调用参数为 self，这意味着它会转移方法接收者的所有权。换句话说，就是这 种方法调用只能被调用 一次
// 
...
一个闭包实现了哪种 Fn 特征取决于该闭包如何使用被捕获的变量，而不是取决于闭包如何捕获它们, 跟是否使用move没有必然联系

    //捕获环境变量
    // 有三种方式, 根据捕获变量方式不同, 将闭包定义为三种 trait:
    // 
    // Fn，表示闭包以不可变借用的方式来捕获环境中的自由变量 ，同时也表示 该闭 包没有 改变环境的能力 ， 并且可以多次调用。闭包接收者是不可变引用, 对应 &self。
    //               若某个闭包参数加了move, 则闭包自动实现 Fn
    ...
    //                      没加 move, 内部有写操作
    //                     修改环境变量以自动实现 FnMut
    // FnOnce， 表示闭包通过转移所有权来捕获环境中的自由变量, 或者根本没有捕获环境变量，意味着该闭包没 有改变环境的能力，只能调用一次(因为该闭包会消耗自身。闭包接收者是值, 并非引用, 对应 self)。
    //              如果你想强制闭包取得捕获变量的所有权，可以在参数列表前添加 move 关键字，
    //              由于所有所有闭包都可以至少调用一次, 所以涉及到捕获环境变量 的所有闭包都实现了 FnOnce 
    //
    // 
    // 三种 trait 包含关系: fn 属于(继承于) fn_mut 属于(继承于) fn_once;
    ...
    // 
    let mut i = 1;
    // 没加 move,捕获的是引用, 闭包为 FnMut, 所以闭包需要声明为 可变
    let mut func =  || {
        i+=1; // 修改了 自由变量,
    };
    func();
...
```rs
// 作为返回值, 必须使用 trait对象
fn square() -> Box<Fn(i32) -> i32> {  // 或者返回类型  Box<dyn Fn(i32) -> i32>
    Box::new(|i| i*i )
}
...
#[derive(Debug)]
struct MyBox(*mut u8);
unsafe impl Send for MyBox {} // 裸指针需要 unsafe

let mb = MyBox(Box::into_raw(Box::new(1)));
...
- Debug  方便用 {:?} 打印
    - 通过 #[derive(Debug)]      // 通过派生实现 trait, 会为泛型参数添加相同的约束 bound, 即添加这行, 会要求泛型也要实现 Debug
...
```rs
// Mutex<T> 互斥锁: 支持跨线程安全共享可变变量的容器, (另一种方法是使用线程本地变量 thread_local!{})
// 同时只允许一个线程访问内部数据, 类似 线程安全版本的 RefCell, 本质是一个 struct
...
fn sum_of_squares(input: &[i32]) -> i32 {
    input.par_iter()// 返回一个不可变的并行迭代器类型
        .map(|&i| i * i).sum()
}
fn increment_all(input: &mut [i32]) {
   
    input.par_iter_mut() // 回一个可变的并行迭代器
...
使用 .await 等待异步函数执行完, 若需要 await 的函数对应的 future 是 pending 状态, 会让出cpu控制权, 继续往下执行其他代码
...
使用 FFi 绑定到其他语言的 gui(c/cpp)
    Windows-rs 微软官方库
    gtk-rs (gtk 很成熟)
    fltk-rs 轻量级, 带有一个 fluid ui 设计工具

web 方案
    Taui  https://github.com/tauri-apps/tauri
