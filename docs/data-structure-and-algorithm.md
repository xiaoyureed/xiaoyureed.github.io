...
O(logn) - 一层循环, 每次循环 n 会除以 2, (或者每次循环步长会乘以 2)
...
将数列分为有序和无序两个部分, 不断从无序的部分拿到元素, 插入到有序的部分中去
    ...
    T: Ord + Clone,
{
    ...
    for item in arr.iter().cloned() {
        // 遍历准备返回的有序数列, 找到比 ele 大的元素, 吧 ele 插入到该元素前面

        let n_len = result.len();
        for i in 0..=n_len {
            if i == n_len || result[i] > item {
...
- 从数列中取一个基准数, 将大于它的数放到右边, 小于它的放到左边
- 对左右两部分重复此操作, 知道各区间内只有一个数
...
重复走访数列, 每次比较两个元素, 如果顺序不对, 就交换他们俩的位置, 走访数列的工作是重复进行的, 知道所有元素有序, 此时停止走访
