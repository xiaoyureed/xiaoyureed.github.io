...
开放平台服务形式:
- 开发者请求时，开放平台返回对应的H5链接, 然后直接在开放平台的H5链接内进行服务。这种形式的开放在支付宝的服务中就有很多，比如社保公积金查询、汽车服务等。
    优势: 对于开发者来说，开发成本极低，基本上没有什么开发量
    劣势: 开发者无法直接获取自己系统用户实际的业务情况，比如下单量、下单金额等，完全依靠平台提供的数据；相应的页面的风格无法变更
- 开发者请求时，根据API的函数传入对应的参数，平台返回对应的数据内容
    优势：开发者可以按照自己系统的设计规范进行呈现，使用户无法感知相关的资源和服务是第三方平台提供的。对于有交易类型及分成的业务，开发者可以自己掌握相关订单数据，不用完全受制于平台方提供的数据；
    劣势：开发成本相对较高，开发者需要详细了解业务规则及逻辑，同时还要进行前端页面设计和开发
...
目标: 拉新、促活，还是增收，或者推广品牌

形式: 
    - 内容形式: 抽奖、话题传播、小游戏、领取折扣
    - 产品形态: 微信H5、小程序、APP链接
...
预算管控机制
    尽量可以让运营人员实时调控中奖率、奖品数量、奖品类型, 将预算控制在可控范围内，并且又不影响参与用户体验。

测试机制
    - 活动不仅仅要依靠专业测试人员进行测试，还需要发动广大小白用户进行试玩，范围越广泛越好，比如：我们之前就是项目组玩玩，后面老实发生问题，以至于后面我们新发布活动前，都会要求集团行政发通知全员参与活动，有问题项目组收集起来
    - 活动往往参与时间比较集中，因此并发应该是重点考虑的，必须进行压力测试，避免因为并发过大造成系统崩溃
    - 测试阶段要配合运营整理好一份详细的常见问题及话术给客服，让客服详细掌握活动的情况
```
